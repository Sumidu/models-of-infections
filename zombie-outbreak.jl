...
if isfile("luebeck.json")
    println("File exists. Using file.")
else
    println("Downloading map of Lübeck")
    OSM.download_osm_network(
        :point;
        point = LightOSM.GeoLocation(53.866781, 10.686955), # center of Lübeck
        radius = 2, # 5 kilometers around it
        save_to_file_location = "luebeck.json"
    )
end
...
function initialise(; seed = 1234)
    ...
    for id in 1:100
        ...
        speed = rand(model.rng) * 2.5 + 2.0 # Random speed from 2-7kmph
    ...
    start = OSM.nearest_road((53.866281, 10.680175), model)
    finish = OSM.nearest_node((53.861550, 10.688594), model)
...
## Here, we begin generating the output

using InteractiveDynamics
using CairoMakie
ac(agent) = agent.infected ? :red : :black
as(agent) = agent.infected ? 12 : 8
model = initialise()

@time abmvideo("outbreak.mp4", model, agent_step!;
        title = "Zombie outbreak", framerate = 30, frames = 600, as, ac)